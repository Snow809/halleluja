...
export function useMyDocuments() {
  return useQuery({
    ...
    queryFn: () =>
      api.get<{ hrDocs: HrDocument[]; generated: GeneratedDocument[] }>("/employees/me/documents"),
  });
}
...
export function useDocumentRequests() {
  return useQuery({
    ...
    queryFn: () =>
      api.get<{ requests: HrRequest[]; generated: GeneratedDocument[] }>("/employees/me/documents/requests"),
  });
}
...
export function useRequests(status?: string, kind?: string) {
  ...
  return useQuery({
    queryKey: [...keys.requests, status, kind],
    queryFn: () => api.get<HrRequest[]>(`/employees/requests?${query.toString()}`),
  });
}
...
export function useChatMutation() {
  return useMutation({
    mutationFn: (input: { question: string; conversationId?: string }) =>
      api.post<ChatResponse>("/chat/ask", input),
  });
}
